...
class ViewController: UIViewController, ARSCNViewDelegate, UIPopoverPresentationControllerDelegate, VirtualObjectSelectionViewControllerDelegate {
    ...
    @objc func startRecording() {
        ...
        if recorder.isAvailable {
            ...
            debugPrint("recording unavailable")
        }
    }
    ...
    func _setObjectScale(_ x:Float, _ y:Float, _ z:Float) {
        guard let virtualObject = virtualObject else { return }
        
        virtualObject.scale = SCNVector3(x,y,z)
    ...
    func _printMeasurements() {
        ...
        let zInches = virtualObject.scale.z * length * inchesPerMeter
        
        let message = NSString(format: "Measurement: %.2f\" x %.2f\" x %.2f\"", xInches,yInches,zInches)
        textManager.showMessage(message as String, autoHide: false)
    }
